...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        setError("Registered but failed to sign in. Please log in.");
        router.push("/login");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center px-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        .text-gradient { background: linear-gradient(135deg, #818cf8 0%, #c084fc 50%, #f472b6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
      ...
      <div className="w-full max-w-md relative">
        ...
        <div className="text-center mb-8">
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif" }} className="text-3xl font-extrabold mb-2">Create account</h1>
          <p className="text-white/40 text-sm">Join thousands helping each other every day</p>
        </div>
